...
export const RunningPlChart = ({
  ...
  const domain = useMemo(() => {
    ...
    const end =
      days === null ? Math.max(...stamps) : until === null ? Date.now() : rangeEnd(until);
  ...
  const plotPoints = useMemo(() => {
    ...
    return series.map((p) => ({
      x: domain === null ? 50 : pctFromRatio((Date.parse(p.date) - domain.start) / domain.span),
  ...
  const resolveIndex = (clientX: number, rect: DOMRect): number => {
    if (series.length <= 1 || domain === null) return 0;
